...
public class SQL {
	...
	public String isDuplicatedByKey(String tableName,String key,String value,String valueType){
		...
		try{
			...
			case "int":
				ps.setInt(1,GraceJava.str2int(value));
				break;
				
			case "Timestamp" :
				ps.setTimestamp(1, GraceJava.str2date(value));
				break;
			
			case "double":
				ps.setDouble(1, GraceJava.str2double(value));
				break;
	...
	public boolean deleteByKey(String tableName,String MatchKeys,String MatchValues,String MatchValueTypes){
		...
		try{
			...
			for(String type :TypeList){
				GraceJava.PrepareSql(ps, i, ValueList[i-1], type);
				++i;
				}
	...
	{	
		...
		try{
			...
			results.next();
			mapping=GraceJava.MapFill(results,SelectedKeys.split(","),this.colMAPtype);
		}
		catch(SQLException e){
			e.printStackTrace();
		}
	...
	public ArrayList<Map<String,String>> SelectBat(String tableName,String SelectedKeys,String MatchKeys,String MatchValues,String MatchValueTypes)
	{	
		ArrayList<Map<String,String>> mappings=new ArrayList<Map<String,String>>();
		String Judges=MatchKeys.replaceAll(",", " =? and ")+"=?";
		Connection conn=getConn();
		String sql ="SELECT "+SelectedKeys+" FROM "+tableName+" WHERE "+Judges;
		try{
			...
			ResultSet results = ps.executeQuery();
			while(results.next()){
				mappings.add(GraceJava.MapFill(results,SelectedKeys.split(","),this.colMAPtype));
			}
		}
		catch(SQLException e){
			e.printStackTrace();
...
public boolean changeInfo(String tableName,int id,String Keys,String MatchValues,String MatchValueTypes){
		...
		GraceJava.PrepareSql(ps, i, GraceJava.int2str(id), "int");
		if(secureExecute(conn,ps)){
			ret=true;
		}
	...
	public String TypesOfKeys(String Keys){
		String[] keys=Keys.split(",");
		String[] types=new String[keys.length];
		int index=0;
		for(String key : keys){
			types[index] = colMAPtype.get(key);
			++index;
		}
		String MatchValueTypes=String.join(",", types);
		return MatchValueTypes;
	}
